...
function parseRound(round: string, group?: string): { stage: string; group_name: string | null } {
  ...
  if (/Round of 32/i.test(round) || /1\/16.finals?/i.test(round))  return { stage: 'r32',   group_name: null }
  if (/Round of 16/i.test(round) || /1\/8.finals?/i.test(round) || /last.?16/i.test(round))  return { stage: 'r16',   group_name: null }
  if (/Quarter.final/i.test(round) || /1\/4.finals?/i.test(round)) return { stage: 'qf',   group_name: null }
  if (/Semi.final/i.test(round) || /1\/2.finals?/i.test(round))   return { stage: 'sf',    group_name: null }
