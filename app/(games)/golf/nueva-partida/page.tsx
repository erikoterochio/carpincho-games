...
const C = {
  ...
  borderActive: 'var(--accent)', primary: 'var(--accent)',
  text: 'var(--text)', muted: 'var(--text-soft)',
    ...
    <>
      ...
      <div style={{ background: C.bg, minHeight: '100vh', fontFamily: FONT, color: C.text }}>
        <div style={{ maxWidth: 480, margin: '0 auto' }}>
          ...
          {/* Botón flotante */}
          <div style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 480, padding: '12px 18px 30px', background: 'linear-gradient(to top, var(--bg) 75%, transparent)' }}>
            {step < 4 ? (
              <button disabled={!canNext()} onClick={() => setStep(s => s + 1)}
                style={{ width: '100%', padding: '15px', background: canNext() ? C.primary : 'var(--surface-2)', color: canNext() ? '#ffffff' : C.muted, border: 'none', borderRadius: 12, fontFamily: FONT, fontSize: 15, fontWeight: 700, cursor: canNext() ? 'pointer' : 'not-allowed', transition: 'all 0.2s' }}>
                {step === 3 ? 'Ver resumen →' : 'Continuar →'}
              </button>
            ) : (
              <button disabled={creating} onClick={handleCreate}
                style={{ width: '100%', padding: '15px', background: creating ? 'var(--surface-2)' : C.primary, color: creating ? C.muted : '#ffffff', border: 'none', borderRadius: 12, fontFamily: FONT, fontSize: 15, fontWeight: 700, cursor: creating ? 'not-allowed' : 'pointer' }}>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
              {newCourse.holes.map((h: HoleInput, i: number) => (
                <div key={i} style={{ background: 'var(--surface-2)', border: `1px solid ${C.border}`, borderRadius: 8, padding: '7px 10px', display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 3, marginLeft: 'auto' }}>
                    ...
                    <input type="number" min={1} max={18} value={h.stroke_index}
                      ...
                      style={{ width: 32, padding: '2px 3px', background: 'var(--bg)', border: `1px solid ${C.border}`, borderRadius: 5, color: C.text, fontSize: 11, textAlign: 'center' }} />
                  </div>
                </div>
              ))}
            </div>
                  ...
                  <div style={{ padding: '10px 14px', borderTop: `1px solid ${C.border}`, background: C.card, display: 'flex', gap: 10, alignItems: 'flex-end' }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <select value={active.hole} onChange={(e: any) => updateContest(type, 'hole', parseInt(e.target.value))}
                        ...
                        {holeNumbers.map((h: number) => <option key={h} value={h} style={{ background: 'var(--surface)' }}>Hoyo {h}</option>)}
                      </select>
                    </div>
                    <div style={{ flex: 2 }}>
              ...
              <button onClick={() => toggle(key)}
                ...
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 5, flexWrap: 'wrap' }}>
                    ...
                    <span style={{ fontSize: 10, color: C.muted, background: 'var(--surface-2)', borderRadius: 5, padding: '2px 7px' }}>
                ...
                <div style={{ padding: '0 14px 14px 46px', display: 'flex', alignItems: 'center', gap: 12, borderTop: `1px solid ${def.color}30` }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <button onClick={() => onFormatConfig(p => ({ ...p, fourball_clasico: { max_hcp_diff: Math.max(0, (p.fourball_clasico?.max_hcp_diff ?? 5) - 1) } }))}
                      style={{ width: 28, height: 28, borderRadius: 6, border: `1px solid ${C.border}`, background: 'var(--surface-2)', color: C.text, fontSize: 16, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>−</button>
            ...
            {players.map((p: PlayerInput) => {
              ...
              return (
                <div key={p.tempId} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <span style={{ fontSize: 12, color: C.muted, background: 'var(--surface-2)', borderRadius: 6, padding: '2px 8px' }}>
                        ...
                        <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', paddingTop: 6, borderTop: `1px solid ${C.border}` }}>
                          {unassigned.map((p: PlayerInput) => (
                            <button key={p.tempId} onClick={() => assignPlayer(absIdx, p.tempId)}
                              style={{ fontSize: 11, color: C.text, background: 'var(--surface-2)', border: `1px solid ${C.border}`, borderRadius: 6, padding: '4px 9px', cursor: 'pointer' }}>
                              + {p.display_name}
                            </button>
                          ))}
                        </div>
...
function SummaryCard({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, overflow: 'hidden' }}>
      <div style={{ padding: '7px 14px', background: 'var(--surface-2)', borderBottom: `1px solid ${C.border}` }}>
