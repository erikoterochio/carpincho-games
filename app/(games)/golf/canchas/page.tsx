              ...
              <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: '16px', display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 5 }}>
                    {form.holes.map((h, i) => (
                      <div key={i} style={{ background: 'var(--surface-2)', border: `1px solid ${C.border}`, borderRadius: 7, padding: '6px 9px', display: 'flex', alignItems: 'center', gap: 5 }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 2, marginLeft: 'auto' }}>
                          ...
                          <input type="number" min={1} max={18} value={h.stroke_index}
                            ...
                            style={{ width: 30, padding: '1px 2px', background: 'var(--bg)', border: `1px solid ${C.border}`, borderRadius: 4, color: C.text, fontSize: 10, textAlign: 'center' }} />
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <button onClick={handleCreate} disabled={saving || !form.name.trim()}
                  style={{ padding: '13px', background: saving || !form.name.trim() ? 'var(--surface-2)' : C.primary, border: 'none', borderRadius: 10, fontFamily: FONT, fontSize: 14, fontWeight: 700, color: saving || !form.name.trim() ? C.muted : '#ffffff', cursor: saving || !form.name.trim() ? 'not-allowed' : 'pointer' }}>
...
function CourseCard({ course, holes, expanded, onToggle, isOwner }: {
  ...
  return (
    <div style={{ background: 'var(--surface)', border: `1px solid ${expanded ? 'var(--accent)' : 'var(--border)'}`, borderRadius: 14, overflow: 'hidden', transition: 'border-color 0.15s' }}>
      {/* Header clickeable */}
      <button onClick={onToggle}
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--text)', marginBottom: 3 }}>{course.name}</div>
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            {course.city && <span style={{ fontSize: 11, color: 'var(--text-soft)' }}>{course.city}</span>}
            <span style={{ fontSize: 11, color: 'var(--text-soft)' }}>{course.total_holes} hoyos</span>
            {course.par && <span style={{ fontSize: 11, color: 'var(--text-soft)' }}>Par {course.par}</span>}
            {course.rating && <span style={{ fontSize: 11, color: 'var(--text-soft)' }}>Rating {course.rating}</span>}
            {course.slope && course.slope !== 113 && <span style={{ fontSize: 11, color: 'var(--text-soft)' }}>Slope {course.slope}</span>}
          </div>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
          {isOwner && <span style={{ fontSize: 9, fontWeight: 700, color: 'var(--accent)', letterSpacing: 0.8 }}>TUYA</span>}
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none"
            style={{ transform: expanded ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }}>
            <path d="M6 9l6 6 6-6" stroke="var(--text-soft)" strokeWidth="2" strokeLinecap="round"/>
          </svg>
        </div>
      </button>
      ...
      {/* Detalle de hoyos */}
      {expanded && (
        <div style={{ borderTop: '1px solid var(--border)', padding: '12px 16px', overflowX: 'auto' }}>
          {holes === null ? (
            <p style={{ fontSize: 12, color: 'var(--text-soft)', textAlign: 'center', padding: '10px 0' }}>Cargando hoyos...</p>
          ) : holes.length === 0 ? (
            <p style={{ fontSize: 12, color: 'var(--text-soft)', textAlign: 'center', padding: '10px 0' }}>Sin hoyos cargados</p>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11, fontFamily: FONT, minWidth: 380 }}>
              <thead>
                <tr>
                  {['H', 'Par', 'Hcp', '⬛', '🔵', '⬜', '🟡', '🔴'].map(h => (
                    <th key={h} style={{ padding: '4px 6px', color: 'var(--text-soft)', fontWeight: 700, textAlign: 'center', borderBottom: '1px solid var(--border)', fontSize: 10 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {holes.map(h => (
                  <tr key={h.hole_number}>
                    <td style={{ padding: '5px 6px', textAlign: 'center', color: 'var(--text-soft)', fontWeight: 700 }}>{h.hole_number}</td>
                    <td style={{ padding: '5px 6px', textAlign: 'center', color: 'var(--text)', fontWeight: 700 }}>{h.par}</td>
                    <td style={{ padding: '5px 6px', textAlign: 'center', color: 'var(--text-soft)' }}>{h.stroke_index}</td>
                    {['distance_black','distance_blue','distance_white','distance_yellow','distance_red'].map(key => (
                      <td key={key} style={{ padding: '5px 6px', textAlign: 'center', color: 'var(--text-soft)' }}>
                        {(h as any)[key] ?? '—'}
                      </td>
                    ))}
                  </tr>
                ...
                <tr style={{ borderTop: '1px solid var(--border)' }}>
                  <td style={{ padding: '5px 6px', textAlign: 'center', color: 'var(--text-soft)', fontWeight: 700 }}>Σ</td>
                  <td style={{ padding: '5px 6px', textAlign: 'center', color: 'var(--text)', fontWeight: 700 }}>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--text-soft)', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 6 }}>{label}</div>
      {children}
    </div>
  )
}