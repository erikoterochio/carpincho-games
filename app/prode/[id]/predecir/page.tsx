...
export default function PredecirPage() {
  ...
  const handlePickChange = (matchId: string, side: 'h'|'a', value: string) => {
    ...
    if (!match) return
    if (Date.now() >= new Date(match.kickoff).getTime() - 600000 || (match.stage === 'group' && isDeadlinePast)) return
  ...
  const GroupMatchRow = ({ m }: { m: Match }) => {
    ...
    const started = Date.now() >= new Date(m.kickoff).getTime() - 600000
  ...
  const MatchCard = ({ m }: { m: Match }) => {
    ...
    const matchLocked = Date.now() >= new Date(m.kickoff).getTime() - 600000 || (m.stage === 'group' && isDeadlinePast)
